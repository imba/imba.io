...
export var language = {
	...
	keywords: [
		...
		'var', 'let', 'const', 'require', 'isa', 'await','static'
	],
	...
	methodName: /[A-Za-z\_\$][A-Za-z\d\$\_]*(\-+[\w\$]+)*[\?\!\=]?/
	identifier: /[a-z_][A-Za-z\d\_\$]*(\-+[\w\$]+)*[\?\!]?/
	...
	tokenizer: {
		...
		body: [
			...
			[/\@[a-zA-Z_\$]\w*(\-\w+)*[\?\!]?/, 'variable.instance'],
			[/\$\w+\$/, 'identifier.env'],
			[/\$\d+/, 'identifier.special'],
			[/\$[a-zA-Z_\$]\w*(\-\w+)*/, 'identifier.sys'],
			[/[A-Z][A-Za-z\d\-\_]*/, token: 'identifier.const'],
			[/[a-z_][A-Za-z\d\_]*(\-\w+)*[\?\!]?/, token: 'identifier'],
			...
			[/### (css\b.*)/, { token: 'comment', next: '@style_comment', nextEmbedded: 'text/css' }]
			[/### (less\b.*)/, { token: 'comment', next: '@style_comment', nextEmbedded: 'text/less' }]
			[/### (sass\b.*)/, { token: 'comment', next: '@style_comment', nextEmbedded: 'text/css' }]
			[/### (scss\b.*)/, { token: 'comment', next: '@style_comment', nextEmbedded: 'text/css' }]
